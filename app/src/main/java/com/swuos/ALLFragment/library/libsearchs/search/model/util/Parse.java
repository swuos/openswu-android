package com.swuos.ALLFragment.library.libsearchs.search.model.util;
    ...
    public static List<SearchBookItem> getSearchResult(String s) {
        ...
        for (Element aa : elements1) {
            ...
            for (Element dd : cc) {
                ...
                switch (classname) {
                    case "creator":
                        searchBookItem.setWriter(dd.text().replace("—", "").replace("著", ""));
                        break;
                    case "call_number":
                        searchBookItem.setBookSuoshuhao(dd.text().replace("：", ": "));
                        break;
                    case "isbn":
                        searchBookItem.setISBN(dd.text().substring(dd.text().indexOf("标准编码： ") + 6).replace("-", ""));
                        break;
                    case "publisher":
